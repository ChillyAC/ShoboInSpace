...
public var canEndTurnBool : boolean = false;
public var canBombBool : boolean = false;
public var canLaserBool : boolean = false;
...
function Start () {
	...
	startTime = Time.time;
	rescueTime = Random.Range(180, 301);
	...
	if (Input.touchCount > 0) {
		...
		switch (touch.phase) {
			case TouchPhase.Began:	touchStartPos = touch.position;
									...
									if (canLaserBool && upBool && (gameObject.GetComponent("turnManager_tutorial") as turnManager_tutorial).playersTurn == true) {
					                    ...
					                    if (Physics.Raycast(screenRay, hit))
					            			if (hit.collider.gameObject.tag == "enemy") {
					            				...
					            				if ((gameObject.GetComponent("tutorialHandler") as tutorialHandler).phaseID == 8) (gameObject.GetComponent("tutorialHandler") as tutorialHandler).nextPhase();
					            				upBool = false;
					            				canLaserBool = false;
					            			}
					            	}
			...
			case TouchPhase.Moved:	if ((gameObject.GetComponent("turnManager_tutorial") as turnManager_tutorial).playersTurn == true) {
										if (canBombBool && downBool && touch.deltaPosition.magnitude > 30 && touch.deltaPosition.magnitude/touch.deltaTime > 30) ebombBool = true;
										if (canEndTurnBool && upBool && touch.deltaPosition.magnitude > 30 && touch.deltaPosition.magnitude/touch.deltaTime > 30) endTurnBool = true;
									}
									else if ((gameObject.GetComponent("turnManager_tutorial") as turnManager_tutorial).playersTurn == false) {
			...
			case TouchPhase.Ended:	touchEndPos = touch.position;
									...
									if (defenseBool) {
									    if (rightBool && touchEndPos.x < 0.5 * Screen.width) {
									        (player.GetComponent("playerController") as playerController).energy2health();
									        (gameObject.GetComponent("tutorialHandler") as tutorialHandler).nextPhase();
									    }
									    else if (leftBool && touchEndPos.x > 0.5 * Screen.width) {
									        (player.GetComponent("playerController") as playerController).health2energy();
									        (gameObject.GetComponent("tutorialHandler") as tutorialHandler).nextPhase();
									    }
										defenseBool = false;
										downBool = false;
									...
									if (ebombBool && touchEndPos.y > 0.5 * Screen.height && (gameObject.GetComponent("turnManager_tutorial") as turnManager_tutorial).playersTurn == true) {
...
//	GUI.Label(Rect(0.045*Screen.width,0.905*Screen.height,0.4*Screen.width,0.2*Screen.height),"Shobo's ship energy");
	...
	if ((gameObject.GetComponent("turnManager_tutorial") as turnManager_tutorial).playersTurn == true) {
		GUI.contentColor = Color.blue;
